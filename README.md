...
- **Render / low RAM (~512MB):** when **`RENDER=true`** (set by Render) or **`LOW_RAM=1`**, the app defaults to a **smaller embedding model** (`paraphrase-MiniLM-L3-v2`), **`cpu`**, tiny encode/FAISS batches, **`low_cpu_mem_usage`**, and **`gc.collect()`** between batches. Override with `HF_EMBEDDING_MODEL` / batch env vars if needed. For reliable headroom, use a **paid** instance with more RAM.
